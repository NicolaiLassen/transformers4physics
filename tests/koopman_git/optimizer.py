...
class SimErrorOptimizer(object):
    ...
    def __init__(self, model, data, data_size, tvec=None, disc=True, errtype='sim', lr=1e-3, lr_decay=0.95, epochs=10000, batch_size=256, opt_tol=1e-6, debug=False, alpha=10e3):
            ...
            for batch in np.array_split(np.random.permutation(self.data_size), max(int(self.data_size / self.batch_size), 1)):
                ...
                def sim_loss():
                    ...
                    if hasattr(self.model, 'phi_inv'):
                        loss += self.alpha * mse_loss(self.model.phi_inv(Z), self.X[batch, :])
