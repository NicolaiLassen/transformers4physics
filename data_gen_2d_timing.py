...
def d_theta_to_coords(d,theta):
    ...
    return np.array([d*cos(theta_rad),d*sin(theta_rad)])
...
def generate_data_set(
    ...
    for i in range(num_sequences):
        ...
        field = np.zeros(3)
        d = (field_strength_interval[1]-field_strength_interval[0]) * rng.random() + field_strength_interval[0]
        theta = (field_angle_interval[1]-field_angle_interval[0]) * rng.random() + field_angle_interval[0]
        field[0:2] = d_theta_to_coords(d,theta)
...
if __name__ == '__main__':
    ...
    for i in range(len(times)):
        ...
        a = generate_sequence(
            ...
            field_micro_t=[-24.6,4.3,0],
            time_per_step=4e-12,
            show=False,
        )
        done_time = time.time_ns()
        times[i] = done_time-current_time
    times = times * 1e-9
    with open('./Magtense Times.txt', 'w') as f:
        for t in times:
            f.write('{} s \n'.format(t))
        f.write('\nAverage: {} s'.format(times.mean()))
